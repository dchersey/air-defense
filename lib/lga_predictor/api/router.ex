defmodule LgaPredictor.API.Router do
  ...
  # Store the FlightAware AeroAPI key in the Keychain (real-time flight routes).
  # Body: {"key": "..."}.
  post "/api/aeroapi_key" do
    case conn.body_params do
      %{"key" => key} when is_binary(key) and key != "" ->
        case LgaPredictor.Routes.put_key(key) do
          :ok -> send_json(conn, 200, %{ok: true, aeroapi_key_present: true})
          {:error, reason} -> send_json(conn, 500, %{error: reason})
        end
